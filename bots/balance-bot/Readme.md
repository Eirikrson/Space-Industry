...
The Electrolyser, Fuel Processor, and Farm Module use configurable minimum targets. The bot sets Hydrogen to 100, Oxygen to 50, Fuel to at least 90 percent of tank capacity, and Food to at least 40 while increasing it with crew size. The Electrolyser always produces both gases together until both minimums are satisfied.
...
Smelter and Assembler targets are calculated from the current milestone and its directly following build step. The bot expands recipes back to their required plates and ore, keeps ore unprocessed when upcoming research or buildings need it directly, and lowers obsolete production targets again instead of maintaining fixed stockpiles.
...
After a completed or failed run the terminal closes automatically. A manually stopped run still asks whether its report should be kept, then closes after the answer.
